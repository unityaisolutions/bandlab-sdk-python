...
from bandlab_sdk import BandlabSDK, AsyncBandlabSDK, DefaultAioHttpClient
from bandlab_sdk._utils import is_dict
...
logging.getLogger("bandlab_sdk").setLevel(logging.DEBUG)
...
def client(request: FixtureRequest) -> Iterator[BandlabSDK]:
    strict = getattr(request, "param", True)
    if not isinstance(strict, bool):
        raise TypeError(f"Unexpected fixture parameter type {type(strict)}, expected {bool}")

    with BandlabSDK(base_url=base_url, bearer_token=bearer_token, _strict_response_validation=strict) as client:
        yield client


@pytest.fixture(scope="session")
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncBandlabSDK]:
